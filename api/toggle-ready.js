import storage from '../lib/storage.js';

export default async function handler(req, res) {
  // Configurar CORS
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');

  if (req.method === 'OPTIONS') {
    return res.status(200).end();
  }

  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }

  try {
    const { roomCode, playerId } = req.body;
    
    if (!roomCode || !playerId) {
      return res.status(400).json({ error: 'Código de sala y ID de jugador requeridos' });
    }

    const room = await storage.getRoom(roomCode);
    
    if (!room) {
      return res.status(404).json({ error: 'Sala no encontrada' });
    }

    if (room.started) {
      return res.status(400).json({ error: 'La partida ya comenzó' });
    }

    // Buscar el jugador
    const player = room.players.find(p => p.id === playerId);
    
    if (!player) {
      return res.status(404).json({ error: 'Jugador no encontrado' });
    }

    // El host no puede cambiar su estado (siempre está listo)
    if (player.isHost) {
      return res.status(400).json({ error: 'El host siempre está listo' });
    }

    // Toggle el estado de ready
    player.ready = !player.ready;
    
    await storage.updateRoom(roomCode, room);

    console.log(`[TOGGLE-READY] ${player.name} ahora está ${player.ready ? 'listo' : 'no listo'}`);

    res.status(200).json({
      success: true,
      ready: player.ready,
      readyPlayers: room.players.filter(p => p.ready).length,
      totalPlayers: room.players.length
    });

  } catch (error) {
    console.error('Error al cambiar estado de listo:', error);
    res.status(500).json({ error: error.message });
  }
}

